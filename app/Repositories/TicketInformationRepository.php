...
class TicketInformationRepository
{
            ...
            if (isset($get_booking_data->data->id)) {
                if (isset($get_booking_data)) {
                    ...
                    if ($cancelticket->origin == 'ODBUS') {
                        ...
                        $busName = $getBus_id[0]->Bus->name;
                        $busNumber = $getBus_id[0]->Bus->bus_number;
                    } elseif ($cancelticket->origin == 'DOLPHIN') {
                    ...
                    $bus_id = $getBus_id[0]->bus_id;
                    $smsData = array(
                        'customer_name' => $request['bookingInfo']['bookingDetail'][0]['passenger_name'],
                        'customer_mob' => $request['customerInfo']['phone'],
                        'pnr' => $pnr,
                        'busdetails' => $busName . '-' . $busNumber,
                        'busname' => $busName,
                        'vechicle_no' => $busNumber,
                        ...
                        'seat' => implode(',', $request['bookingInfo']['seat_names']),
                        'refundAmount' => 0
                    );
                    ...
                    // return $request['bookingInfo']['user_id'];
    ...
    public function getDetailsSms($request)
    {
        ...
        if ($action == 'smsToCustomer') {
            ...
            if ($sms_Details == '[]') {
                ...
                $passanger_name = $passengerDetails[0]->passenger_name;
                $i = 0;
                $m = 0;
                $f = 0;
                $O = 0;
    ...
    public function save_customSMS($data)
    {
        ...
        $smsdata = array(
            ...
            'pnr' => $data['form_data']['pnr_no'],
            'source' => $data['form_data']['source'],
            'destination' => $data['form_data']['destination'],
            'boarding_point' => $data['form_data']['boarding_point'],
            'dropping_point' => $data['form_data']['dropping_point'],
            'busname' => $data['form_data']['bus_name'],
            'vechicle_no' => $data['form_data']['vechicle_no'],
            'doj' => $data['form_data']['doj'],
            'dep' => $data['form_data']['dep'],
            'passanger' => $data['form_data']['passanger_name'],
            'seat' => $data['form_data']['seat_no'],
            'sleeper' => $data['form_data']['seat_no'],
            'fare' => $data['sms_data']['fare'],
            'conductor_no' => $data['form_data']['conductor_no'],
        );
        // return $smsdata;

        return $SMS = $this->msg91Service->customer_ticket_booking($smsdata);
