...
class ApiClientWalletRepository
{
    ...
    public function apiClientTotalTransactions($request)
    {
        ...
        if ($user_id) {
            ...
            //                 $main['cancel_charges'] = $w['deduction_amount'];
            //             }
            ...
            if (count($data) > 0) {
                ...
                foreach ($data as $e => $w) {
                    ...
                    if ($e == 0) {
                        if ($w['transaction_type'] == 'c') {
                            $lastClosingBalance = $w['balance'] - $w['amount'];
                        } else {
                            $lastClosingBalance = $w['balance'] + $w['amount'];
                        }
                    }

                    $main['opening_balance'] = $lastClosingBalance;
                    ...
                    if ($w['booking_id'] != null) {
                        ...
                        if ($w['type'] == 'Refund') {
                            ...
                            if (
                                isset($data[$e + 1]) &&
                                $data[$e + 1]->type == 'CancelCommission' &&
                                $w['booking_id'] == $data[$e + 1]->booking_id
                            ) {

                                $main['Refund'] =
                                    $w['amount'] + $data[$e + 1]->amount;
                        ...
                        // STORE ONLY NON COMMISSION ROWS
                        if (
                            $w['type'] != 'Commission' &&
                            $w['type'] != 'CancelCommission'
                        ) {

                            // UPDATE LAST CLOSING BALANCE
                            $lastClosingBalance =
                                $main['closing_balance'];
