...
class BusRepository
{
    ...
    public function __construct(
        BusDisplayInfo $BusDisplayInfo,
        Bus $bus,
        BusSchedule $busSchedule,
        BusType $busType,
        BusSitting $busSitting,
        BusStoppage $busStoppage,
        BusAmenities $busAmenities,
        Amenities $amenities,
        BoardingDroping $boardingDroping,
        User $user,
        ...
        public function BusData($request)
        {
            ...
            if ($data) {
                foreach ($data as $v) {
                    ...
                    //  Total Booked Seats
                    $v['total_booked'] = \DB::table('booking_detail as bd')
                    ...
                    //  Total Blocked Seats
                    $v['total_blocked'] = \DB::table('bus_seats')
                        ->where('bus_id', $v->id)
                        ->where('type', 2)
                    ...
                    //  Total Open Seats
                    $v['total_open'] = \DB::table('bus_seats')
                        ->where('bus_id', $v->id)
                        ->where('type', 1)
                        ->whereNotNull('operation_date')
                        ->where('status', 1)
                    ...
                    //  Total Extra Seats
                    $v['total_extra'] = \DB::table('bus_seats')
                        ->where('bus_id', $v->id)
    ...
    public function GetBusList($data)
    {
        ...
        } elseif (isset($data['bus_operator_id']) && $data['bus_operator_id'] && !isset($data['route'])) {
        ...
        } elseif (isset($data['bus_operator_id']) && isset($data['route'])) {
    ...
    public function allCouponBusList($id)
    {
        ...
        } elseif ($coupon_detail->source_id == null && $coupon_detail->destination_id == null && $coupon_detail->bus_operator_id != null) {
