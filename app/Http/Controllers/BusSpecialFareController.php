...
class BusSpecialFareController extends Controller
{
    ...
    public function createBusSpecialFare(Request $request)
    {
        ...
        $validation = $this->busSpecialFareValidator->validate($data);
    ...
    public function updateBusSpecialFare(Request $request, $id)
    {
        ...
        try {
            $data['bus_id'] = $request->input('bus_id'); // add bus_id if provided
            $response = $this->busSpecialFareRepository->update($data, $id);

            return $this->successResponse($response, "Bus Special Fare Updated", Response::HTTP_OK);
        } catch (Exception $e) {
            return $this->errorResponse($e->getMessage(), Response::HTTP_PARTIAL_CONTENT);
        }
    }
    ...
    {
        try {
            $busSpecialFare = $this->busSpecialFareRepository->getById($id);
        } catch (Exception $e) {
            return $this->errorResponse($e->getMessage(), Response::HTTP_NOT_FOUND);
        }
    ...
    {
        try {
            $response = $this->busSpecialFareRepository->getPivotData($id);
        } catch (Exception $e) {
            return $this->errorResponse($e->getMessage(), Response::HTTP_NOT_FOUND);
        }
