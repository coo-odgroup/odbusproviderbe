...
class BusCancelledController extends Controller
{
    ...
    public function __construct(busCancelledService $busCancelledService, 
                               CancelBusValidator $cancelBusValidator,
                               BusCancelledRepository $busCancelledRepository)
    {
    ...
    public function createBusCancelled(Request $request) {
      ...
      // exit;
        ...
        try {
          ...
          $data = $this->busCancelledRepository->savePostData($data);
    ...
    public function busCancelledbyowner(Request $request) 
{
    ...
    if ($busCancelledValidation->fails()) {
        $errors = $busCancelledValidation->errors();
        return $this->errorResponse($errors->toJson(), Response::HTTP_UNPROCESSABLE_ENTITY);
    }
    ...
    if (isset($response['msg']) && $response['msg'] === 'Some seat already booked on') {
        $message = $response['msg'] . ' ' . ($response['dt'] ?? '') . ' for cancellation of bus please contact ODBUS Support team';
        return $this->errorResponse($message, Response::HTTP_PARTIAL_CONTENT);
    }
    ...
    public function updateBusCancelled(Request $request, $id) {
        ...
        try {
            ...
            $response = $this->busCancelledRepository->updatePost($data, $id);
    ...
    public function deleteBusCancelled($id) {
        try {
          ...
          $response = $this->busCancelledRepository->deleteById($id);
